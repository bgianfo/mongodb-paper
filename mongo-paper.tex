...
This could be implemented using a four dimensional geospatial index where three of the dimensions
...
data. A database which supports these kind of queries would be incredibly powerful and would open up many
potential real time applications.

This work will most likely occur in the db/geo folder of the database under a new module named "nd.cpp". The new N-Dimensional
implentenatin will be modeled after the existing 2D implementation, it will be general enough to then replace
the 2D implementation. A number of unit tests will also need to be added to the jstests/geo folder to make sure the implementation is sound.
\newpage
%
